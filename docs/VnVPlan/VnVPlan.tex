...
input handling (\rref{R-Inputs}), NCRF computation (\rref{R-Calculate}), and
output generation for downstream analysis (\rref{R-Output}). Each test area
...
\subsubsection{Input Handling and API Selection (\rref{R-Inputs})}
...
\textbf{Test Case Derivation:} R\ref{R-Inputs} requires the system to accept $e(t)$ as input.
...
\textbf{Test Case Derivation:} R\ref{R-Calculate} requires computation of source-level neural current estimates.
...
\textbf{Test Case Derivation:} \rref{R-Output} requires both outputs in a format suitable for downstream analysis.
...
\textbf{Test Case Derivation:} R\ref{R-Output} requires interoperability with the existing pipeline.
...
\midrule
R\ref{R-Inputs}  & FR-INPUT-01, FR-API-02, FR-API-03 \\
R\ref{R-Calculate} & FR-CALC-01, FR-CALC-02 \\
R\ref{R-Output} & FR-OUT-01, FR-OUT-02 \\
