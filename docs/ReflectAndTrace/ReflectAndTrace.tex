...
Feedback was tracked primarily through GitHub issues and through revision notes in the project documents. The most important change across the project was refining the project from the broad goal of ``integrating NCRFs into the pipeline'' into a concrete integration strategy for the existing TRF-Tools workflow. The problem statement frames NCRFs as a way to address error propagation in the current two-stage TRF-based analysis by estimating spatial and temporal response properties jointly rather than sequentially. In response to feedback and implementation work, the design introduced an \texttt{estimator} abstraction so NCRF-based analysis can be selected through the same experiment pipeline used for baseline TRF analysis. This changed the SRS requirements, the MG module decomposition, the MIS interfaces, the VnV test cases, and the user-facing README.
...
No separate Hazard Analysis document was completed for this project.
...
\textbf{Source} & \textbf{Feedback} & \textbf{Response}\\
\midrule
Professor feedback issue \#15 & MG and MIS feedback was provided as annotated PDF attachments. & Revised the design documents to better align the module decomposition, estimator strategy design, and module interfaces with the implemented TRF-Tools integration.\\
...
Peer review issue \#21 & The link in Section 2 of the MIS should point readers to the correct companion documentation. & Updated the MIS documentation references so readers can locate the related SRS and MG material more reliably.\\
Peer review issue \#22 & The Hardware-Hiding Module in the MIS had a typing issue. & Corrected the relevant Hardware-Hiding Module type/interface description so the documented access routines match the intended data and filesystem behavior.\\
\bottomrule
\end{tabularx}
...
Peer review issue \#13 & NFR-ACC-01 needed a clearer accuracy comparison and acceptance criterion. & Revised the accuracy test description to compare outputs under a fixed configuration and report relative-error metrics where a suitable baseline is available.\\
Peer review issue \#14 & The Software Validation section needed to state whether model validation was in scope. & Clarified that validating the underlying NCRF scientific model is out of scope; the project verifies integration of the existing NCRF model into the TRF pipeline.\\
Professor feedback issue \#4 & Professor feedback on the submitted VnV Plan requested more concrete and feasible test evidence. & Specified the Appleseed BIDS dataset, acoustic-envelope predictor, subject/task assumptions, estimator configuration, lag window, and acceptance checks for NCRF execution.\\
\bottomrule
\end{tabularx}
...
Two approved extras were tackled: user documentation and a code walkthrough. The user documentation extra is represented by the root README. This document explains that the runnable implementation is in the separate TRF-Tools repository, identifies the required \texttt{feature/test-pipeline} branch, lists dependency versions, and describes how to download the Appleseed dataset and stimulus WAV files before generating the predictor.

The code walkthrough extra is represented in the design documentation and supporting demo material. The walkthrough explains the path from \texttt{TRFExperiment.load\_trf(..., estimator=...)} to estimator resolution and backend dispatch. The important implementation point is the routing role of the estimator abstraction: different estimator choices correspond to different analysis methods, so a boosting estimator follows the baseline TRF analysis path while an NCRF estimator follows the NCRF analysis path.
...
At the beginning of the project, I misunderstood the concept of NCRF itself. I initially interpreted the ``NC'' in NCRF as ``non-linear convolution,'' which led to an incorrect early framing of the problem. After correcting this misunderstanding, I spent extra time redefining the problem and revising the Problem Statement and parts of the SRS so they correctly described NCRF as Neuro-Current Response Functions for jointly estimating spatial and temporal response properties. Another issue was that implementation details from TRF-Tools sometimes leaked into the wrong documentation level. For example, the MIS needed enough type detail to be readable, but it should not over-specify internal helper functions beyond what the interface requires.
...
For a similar project, I would create a small architecture note earlier that maps the existing codebase to the SRS/MG/MIS modules. I would also keep a living feedback-response table from the first review onward instead of reconstructing traceability near the end. Finally, I would document the demo environment and data-path assumptions earlier, because those details are central to whether another user can reproduce the work.
...
The relevant course background is the same as the preparation identified in the SRS. Programming was needed because the project integrates with a Python-based analysis pipeline and requires users to run or modify scripts and Jupyter notebooks. Linear algebra, probability, and signal processing were relevant because NCRF/TRF analysis works with time-series stimulus features, EEG/MEG recordings, source-space representations, and model outputs. Neuroscience or biomedical engineering background was also relevant because users need to understand EEG/MEG data organization, BIDS-style datasets, and the goal of estimating spatial and temporal neural responses.
