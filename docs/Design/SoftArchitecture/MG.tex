...
\item[\refstepcounter{ucnum} \uctheucnum \label{ucIO}:] Input/Output devices
  (Input: MEG/EEG data and predictors, Output: ).
...
\item [\refstepcounter{mnum} \mthemnum \label{mEstimator}:] Estimator Resolve Module
\item [\refstepcounter{mnum} \mthemnum \label{mTRF}:] TRF Calculation Module
\end{description}
...
\begin{description}
\item[Secrets:]The detailed environment implemention for the hardware.
\item[Services:] Interfaces with system hardware for data input and dispalay.
...
\item[Secrets:]The workflow-level behaviours that realize the functional
  requirements (input ingestion, estimator invocation, and result preparation)
...
\item[Services:]Coordinates execution of the NCRF/TRF workflow through
  ...
  availability, and estimator choice) and invokes TRF computation via the TRF
  Calculation Module. This layer isolates requirement-driven behaviour from core
...
\item[Secrets:]The accepted schema and storage conventions for predictors and
 trial metadata.
\item[Services:]Loads and preprocess MEG/EEG signals, validates metadata. 
\item[Implemented By:] \progname{}
\item[Type of Module:] Library
\end{description}

\subsubsection{Estimator Resolve Module (\mref{mEstimator})}

\begin{description}
\item[Secrets:] The strategies for how to resolving parameters in estimators and applying to experiment.
\item[Services:] Accept 'boosting' and 'ncrf' two types of estimator which defined by the user and make it effective.
\item[Implemented By:] \progname{}
\item[Type of Module:] Library
\end{description}


\subsection{Software Decision Module (\mref{mTRF})}

\begin{description}
\item[Secrets:] The impletion of calcualtion for TRF with different estimation parameters.
\item[Services:] Compute TRF model outputs given preprocessed MEG/EEG data,
  ...
  metrics needed for downstream reporting and output generation.
  % Changes in these modules are more likely to be motivated by a desire to
  % improve performance than by externally imposed changes.
\item[Implemented By:] Elbrain (Python backend)
\end{description}
...
R1 (\textit{R-Inputs}) & \mref{mInput}, \mref{mHH}\\
R2 (\textit{R-Calculate}) & \mref{mEstimator}, \mref{mTRF}\\
R3 (\textit{R-Output}) & \mref{mTRF}, \mref{mEstimator}\\
...
The modules for Integrating NCRF into existing pipeline communicates via structured API calls, and function attributes passing, allowing for clean and efficient data flow for data processing and estimator handling.
...
\begin{table}[H]
  ...
  \midrule
  Hardware-Hiding Module & \mref{mHH} & 2026-03-16 & 2026-03-19 \\
  Input Format Module & \mref{mInput} & 2026-03-18 & 2026-03-22 \\
  Estimator Resolve Module & \mref{mEstimator} & 2026-03-21 & 2026-03-26 \\
  TRF Calculation Module & \mref{mTRF} & 2026-03-24 & 2026-03-30 \\
