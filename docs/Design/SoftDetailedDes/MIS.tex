...
\midrule
April 22, 2026 & 1.3 & Revisions based on peer reviews and professor's feedback\\
April 21, 2026 & 1.2 & Refinements based on professor's feedback\\
...
\href{https://daffodil404.github.io/yanyu_wu_cas741/SRS/SRS.pdf}{SRS}
and
\href{https://daffodil404.github.io/yanyu_wu_cas741/Design/SoftArchitecture/MG.pdf}{MG}.
...
dataset & Dataset & tabular/epoch data object containing response variables, predictor columns, event metadata, and time axes\\
predictor code & PredictorCode & structured predictor request with string field \texttt{key} naming the dataset column to be filled\\
estimator policy & Estimator & strategy/configuration object that identifies an analysis method and its estimator-specific options\\
\bottomrule
\end{tabular} 
\end{center}
...
\texttt{Path}. The additional project-level types are \texttt{Dataset},
\texttt{PredictorCode}, and \texttt{Estimator}. In this project, stimulus features are modeled as
...
for predictors, intermediate artifacts, and outputs. A \texttt{Dataset}
represents the epoch table passed between pipeline stages, a
\texttt{PredictorCode} carries the predictor identity and its \texttt{key}
field for naming the output column, and an \texttt{Estimator} is a Strategy
contract for describing the analysis method, defaults, and option-resolution
rules for concrete estimator policies such as boosting and NCRF. Functions are specified by explicit input/output type signatures,
followed by behavioral requirements and exception conditions where applicable.
...
\hline
\texttt{RawSource.\_load} & \texttt{subject: str}, \texttt{recording: str}, \texttt{preload: bool|-1} & \texttt{Raw} object loaded from EEG/MEG file & \texttt{FileNotFoundError}, \texttt{OSError} (reader I/O errors) \\
\texttt{save.pickle} & \texttt{obj: Any}, optional \texttt{dest: Path|str|None}, optional \texttt{protocol: int} & \texttt{None}; pickle file written to disk & \texttt{OSError}, \texttt{RuntimeError}, pickle serialization errors \\
\hline
\end{tabular}
\end{center}
...
\item exception: Raises \texttt{OSError} for filesystem write failures, \texttt{pickle.PicklingError} when pickle serialization fails, and \texttt{TypeError} when the object graph contains values that cannot be serialized by pickle.
\end{itemize}
...
\begin{itemize}
\item \texttt{PREDICTOR\_FILE\_EXT}: \texttt{str} = \texttt{".pickle"}
\item \texttt{PREDICTOR\_NAME\_SEP}: \texttt{str} = \texttt{"\textasciitilde"}
\item \texttt{PREDICTOR\_FILE\_TEMPLATE}: \texttt{str} = \texttt{"DATA\_ROOT/\{stimulus\}\textasciitilde\{predictor\}.pickle"}, read as: replace \texttt{\{stimulus\}} with the stimulus name and \texttt{\{predictor\}} with the predictor name, then resolve the resulting filename under the predictor data root. Example: if \texttt{DATA\_ROOT = "/Users/yanyuwoo/Data/Appleseed\_BIDS\_20251216"}, then one instantiated path is \texttt{"/Users/yanyuwoo/Data/Appleseed\_BIDS\_20251216/quiet\textasciitilde envelope.pickle"}.
\end{itemize}
...
\hline
\texttt{load\_epochs} &
\texttt{samplingrate: int|None}, \texttt{data: str}, \texttt{interpolate\_bads: bool|"keep"} &
Dataset &
\texttt{ValueError} \\

\texttt{add\_predictor} &
\texttt{ds: Dataset}, \texttt{code: str|PredictorCode}, optional \texttt{filter\_x: bool|"continuous"}, optional target \texttt{y: str|UTS|NDVar|None} &
\texttt{None} &
...
\item Predictor definitions are registered in the external predictor registry before loading.
...
column \texttt{code.key}, where \texttt{code} is a predictor descriptor object and \texttt{code.key} is its string identifier. Predictor values are aligned to the response time axis
for each epoch/trial.
\item output: None (in-place update of \texttt{ds}).
\item exception:
\begin{itemize}
\item Undefined predictor code (not registered in the external predictor registry).
...
Strategy pattern with a registry-based resolver: M3 (Estimator Resolution Module) defines the estimator-selection interface by resolving an estimator key to a concrete estimator policy before TRF computation. The concrete strategy implementations are the \texttt{boosting} estimator and the \texttt{ncrf} estimator, each of which provides its own parameter set and computation path.
...
\item Estimator objects define an estimator key and option-resolution rules that
produce backend-compatible TRF fitting options.
...
\begin{itemize}
\item \texttt{\_apply\_estimator\_options(estimator: Estimator|None, **kwargs: dict)}:
merge estimator defaults and explicit user options into effective TRF options.
\end{itemize}
