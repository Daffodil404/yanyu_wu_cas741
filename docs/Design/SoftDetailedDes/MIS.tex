...
\texttt{save.pickle} & \texttt{res}: Python object, \texttt{dst}: \texttt{Path} & written pickle file at \texttt{dst} & \texttt{OSError}, \texttt{pickle.PicklingError}, \texttt{TypeError} \\
\hline
\end{tabular}
\end{center}
...
\texttt{ds}: Dataset, \texttt{code}: predictor descriptor object with field \texttt{key}: \texttt{str}, optional \texttt{filter\_x}: \texttt{bool}, optional target \texttt{y}: Dataset &
