...
\newpage
\section{MIS of Input Format Module} \label{MIS:Input} 
% \wss{Use labels for cross-referencing}
...
\section{MIS of Estimator Resolve Module} \label{MIS:Estimator}

\subsection{Module}

Estimator resolution service for selecting estimator objects and producing
effective TRF options.
\subsection{Uses}
...
\subsubsection{Exported Constants}
% Supported estimator identifiers accepted by \texttt{load\_trf}.
\begin{itemize}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\texttt{resolve\_estimator} & 
\texttt{estimator}, \texttt{estimators}, \texttt{opts} &
resolved estimator object and effective TRF options &
\texttt{ValueError} (invalid estimator key or invalid estimator map) \\
\hline
\end{tabular}
\end{center}
...
\noindent \texttt{resolve\_estimator(estimator, estimators, opts)}:
\begin{itemize}
\item transition: None.
\item output: Returns a pair \texttt{(estimator\_obj, effective\_opts)} where
\texttt{estimator\_obj} is the resolved estimator instance (or \texttt{None})
and \texttt{effective\_opts} is the TRF option map after applying estimator
defaults.
Estimator resolution rule:
\[
...
\section{MIS of TRF Calculation Module} \label{MIS:TRF}
