...
\section{MIS of Hardware-Hiding Module} \label{Module} 
...
\hline
\texttt{RawSource.\_load} & \texttt{subject}, \texttt{recording}, \texttt{preload} & MNE \texttt{Raw} object loaded from EEG/MEG file & \texttt{FileNotFoundError}, \texttt{OSError} (reader I/O errors) \\
\texttt{save.pickle} & Python object (\texttt{res}) + destination path (\texttt{dst}) & Pickle file written to disk (TRF cache/result file) & \texttt{OSError}, pickle serialization errors \\
\hline
\end{tabular}
\end{center}
...
Filesystem interface (\texttt{fs}) for reading raw EEG/MEG recordings and writing
serialized outputs.
...
\newpage
\section{MIS of Input Format Module} \label{Module} 
% \wss{Use labels for cross-referencing}
...
\subsection{Module}
MEG/EEG Data Formatter and Experiment Configuration.
...
\section{MIS of Estimator Resolve Module} \label{Module}

\subsection{Module}

Estimator Selection and Parameter Resolution.
\subsection{Uses}
...
\hline
\texttt{load\_trf} & 
\texttt{x}, \texttt{tstart}, \texttt{tstop}, optional \texttt{estimator} (\texttt{str} or \texttt{Estimator}) & 
TRF result object or cache path (\texttt{path\_only=True}) & 
\texttt{ValueError} (invalid estimator key or invalid estimator map), \texttt{IOError} (cache miss with \texttt{make=False}) \\
\texttt{\_trf\_job} & 
effective TRF options (including resolved estimator parameters), data/predictor inputs & 
callable that fits TRF (\texttt{boosting} or \texttt{fit\_ncrf}) or \texttt{None} when linked cached NCRF exists & 
...
None. This module is stateless and computes effective options per request.

\subsubsection{Environment Variables}

\begin{itemize}
\item \texttt{estimators}: optional dictionary mapping estimator names
(\texttt{str}) to estimator objects (\texttt{Estimator} subclasses such as
\texttt{BoostingEstimator} or \texttt{NCRFEstimator}).
\item Filesystem cache for TRF artifacts (read/write via path resolution).
\item Backend fitting functions from Eelbrain and NCRF libraries.
\end{itemize}
...
\begin{itemize}
\item transition: No internal state transition. May create/update TRF cache file
when \texttt{make=True}.
\item output: Returns resolved cached TRF, newly fitted TRF, or TRF path.
Resolution rule:
\[
\texttt{estimator is str} \Rightarrow \texttt{estimator := estimators[estimator]}
\]
Then estimator parameters override explicit options (for example
\texttt{delta}, \texttt{basis}, \texttt{partitions}, \texttt{cv},
\texttt{selective\_stopping}, \texttt{partition\_results}) before locating and
computing the TRF.
\item exception:
\begin{itemize}
...
\begin{itemize}
\item transition: None in module state; returns deferred fitting action.
\item output: Returns a callable for backend fitting:
\begin{itemize}
\item \texttt{partial(boosting, ...)} for boosting path.
\item \texttt{partial(fit\_ncrf, ...)} for NCRF path, where
\texttt{estimator.parameters\_for\_partial()} augments NCRF arguments.
...
\begin{itemize}
\item \texttt{resolve\_estimator(estimator, estimators)}:
\[
(\texttt{estimator} \in \texttt{str} \land \texttt{estimator} \in \texttt{keys(estimators)})
\Rightarrow \texttt{estimators[estimator]}
\]
otherwise raise \texttt{ValueError}.
\item \texttt{apply\_effective\_options(base\_opts, est\_opts)}: merges
estimator option map over explicit call options to produce deterministic cache
key and fitter configuration.
\end{itemize}
...
\section{MIS of TRF Calculation Module} \label{Module}

\subsection{Module}

Estimator Selection and Parameter Resolution.
\subsection{Uses}
Uses M2 for data processing and M3 for estimator configurations fitting.
