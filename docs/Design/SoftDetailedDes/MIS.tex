...
Estimator resolution logic embedded in \texttt{load\_trf} (not a standalone API).
\subsection{Uses}
...
\item Estimator resolution is performed inline in \texttt{load\_trf} before the
fitting job is created.
\end{itemize}
...
Estimator resolution (inline logic) rule:
\[
\texttt{estimator is str} \Rightarrow \texttt{estimator := estimators[estimator]}
\]
If resolved estimator is not \texttt{None}, its parameter map overrides explicit
call options (for example
\texttt{delta}, \texttt{basis}, \texttt{partitions}, \texttt{cv},
\texttt{selective\_stopping}, \texttt{partition\_results}) before passing
effective options to TRF computation in M4.
\item exception:
\begin{itemize}
...
None. In current implementation, estimator resolution is not separated into
named helper functions; it is implemented directly inside
\texttt{load\_trf(...)}.
...
deferred callable created with \texttt{partial(...)} for \texttt{boosting} or \texttt{fit\_ncrf} &
...
\begin{itemize}
\item Filesystem cache for TRF artifacts (read/write via path resolution).
\item Backend fitting functions: Eelbrain \texttt{boosting(...)} and NCRF
\texttt{fit\_ncrf(...)}.
