...
\newcommand{\deftheory}[9][Not Applicable]
{
  ...
  \paragraph{RefName: } \textbf{TM\refstepcounter{theorynum}\thetheorynum: #3} \phantomsection
  \label{#2}
...
\begin{tabularx}{\textwidth}{p{3cm}p{2cm}X}
  ...
  \midrule
  Apr 21, 2026 & 1.3 & Updated based on supervisor feedback\\
  Mar 25, 2026 & 1.2 & Address peer review issues for SRS\\
...
\begin{tabular}{l l l}
  \toprule
  \textbf{symbol} & \textbf{quantity} & \textbf{SI unit name}\\
  \midrule
  \si{\decibel} & sound level & decibel\\
...
\begin{longtable}{l l p{12cm}} \toprule
  ...
  $j_{m,t}$ & A$\cdot$m & Dipole current vector at source m and time t\\
  $v_{m,t}$ & A$\cdot$m & stimulus-independent background activity \\
...
This Software Requirements Specification defines the software system \progname{}.
...
The intended readers of this Software Requirements Specification should have at least upper-year undergraduate preparation in programming, linear algebra, probability, and signal processing, as typically covered in senior undergraduate or introductory graduate courses in neuroscience, machine learning, biomedical engineering, or scientific computing.
...
\begin{itemize}
  ...
  \item \textbf{\hyperref[Sec-pd]{Problem Description}} , which presents the scientific
  problem addressed by the system and the context of NCRF-based analysis.
  \item \textbf{\hyperref[sec-goalstatements]{Goal Statements}}, which summarize the objectives
  the system is required to achieve.
  \item \textbf{\hyperref[sec-scope]{Scope of Requirements}} and
  \textbf{\hyperref[sec-IntendedReader]{Characteristics of Intended Reader}} which clarify the
  applicability and assumed background knowledge.
  \item \textbf{\hyperref[sec-gensysdesc]{General System Description}}, which outlines the system
  boundaries, user interactions, and external dependencies.
  \item \textbf{\hyperref[sec-assumpt]{Assumptions}}, which describe the simplifying
...
should then consult the \textbf{\hyperref[sub-sec-scs]{Solution Characteristics Specification}}
, with particular attention to the \textbf{\hyperref[sec-theoretical]{Theoretical Models}}
and \textbf{\hyperref[sec-instance]{Instance Models}}. \\
...
The user could be a researcher or graduate student works on neuro-science, who will provide time-series stimuli and related neuro data, including datasets organized using the Brain Imaging Data Structure (BIDS) format, and it has been represented by the circle in Fig 1. The rectangle represents the software system (NCRF pipeline), which validates input data organization and compatibility before analysis, and the the arrow indicates the workflow of data.
...
The users of the NCRF pipeline should have at least senior undergraduate or introductory graduate-level preparation in neuroscience or biomedical engineering, including familiarity with EEG/MEG data organization, signal-processing concepts, and standard analysis workflows. Users should also be able to run and modify Python-based analysis scripts or Jupyter notebooks to access the pipeline.

% \plt{This section summarizes the knowledge/skills expected of the user.
%   Measuring usability, which is often a required non-function requirement,
%   requires knowledge of a typical user.  As mentioned above, the user is a
%   different role from the ``intended reader,'' as given in
%   Section~\ref{sec-IntendedReader}.  As in Section~\ref{sec-IntendedReader}, the
%   user characteristics should be specific an unambiguous.  For instance, ``The
%   end user of \progname{} should have an understanding of undergraduate Level 1
%   Calculus and Physics.''}

\subsection{System Constraints}
The system should be built based on the existing TRF experiment pipeline and the related NCRF modules of the Eelbrain software library \citep{BrodbeckEelbrain}. This constraint is required to preserve compatibility with the current research workflow, reuse existing preprocessing and baseline TRF analysis components, and support direct comparison between the integrated NCRF results and prior pipeline outputs. The pipeline will therefore remain Python-based, and its interactive functions will remain available through Jupyter Notebook.
...
\begin{itemize}
  ...
  \item \textbf{Continuous stimulus}: A continuously varying input signal, such as speech or another time-series stimulus feature, that is sampled over time rather than represented as isolated events.
  ...
  \item \textbf{BIDS}: Brain Imaging Data Structure, a standardized format for organizing and describing neuroimaging data \citep{gorgolewski2016bids}.
\end{itemize}
...
\noindent The goal statements are:

\begin{itemize}

    % \item[GS\refstepcounter{goalnum}\thegoalnum \label{G-meaningfulLabel}:] \plt{One
    %     sentence description of the goal.  There may be more than one.  Each Goal
    %     should have a meaningful label.}

  \item[GS1:] Provide source-level characterization of neural responses to continuous stimulus feature signals using the NCRF framework.
  \item[GS2:] Support downstream neuroscience analysis by producing outputs that integrate with the existing research workflow.
...
\begin{itemize}
    ...
    % \item[A\refstepcounter{assumpnum}\theassumpnum \label{A-meaningfulLabel}:]
  ...
  Preprocessed, time-aligned stimulus features and EEG/MEG recordings are available as inputs (used by \iref{IM-NeuralStimResponse}).

  \item[A\refstepcounter{assumpnum}\theassumpnum \label{A2}:]
  Cortical source responses are modeled using linear temporal filters; nonlinear dynamics are not considered (used by \tref{TM-NCRF-Model}, \dref{GD-StimulusDriven}, and \iref{IM-NeuralStimResponse}).

  \item[A\refstepcounter{assumpnum}\theassumpnum \label{A3}:]
  The cortical source space and the corresponding forward model are assumed to be fixed and known (used by \tref{TM-NCRF-Model} and \iref{IM-NeuralStimResponse}).

  \item[A\refstepcounter{assumpnum}\theassumpnum \label{A4}:]
  The brain response to a stimulus is not instantaneous; it occurs with a non-zero time lag (used by \tref{TM-NCRF-Model}, \dref{GD-StimulusDriven}, and \iref{IM-NeuralStimResponse}).
\end{itemize}
...
% #5 description
{%
  The NCRF model defines how the stimulus feature drives the neural source current at cortical source location $m$. The location index $m$ refers to a dipole position in a discretized cortical source space embedded in $\mathbb{R}^3$.
  ...
  (unit: \si{\ampere\metre} per stimulus unit), where $\ell \in \{0,1,\ldots,L-1\}$ is an integer lag index and $L \in \mathbb{N}$ is the total number of discrete lags. Thus, $\ell$ indexes individual lag values, while $L$ specifies the overall lag-window length.
...
% #8 Referenced by
{%
  \dref{GD-StimulusDriven}, \dref{GD-BackgroundActivity}, and \iref{IM-NeuralStimResponse}.
}
% #9 Preconditions
{%
  None.
...
\begin{minipage}{\textwidth}
  ...
  \begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
    \hline
    \rowcolor[gray]{0.9}
    Number& GD\refstepcounter{defnum}\thedefnum \label{GD-StimulusDriven}\\
    \hline
    ...
    $\tau_{m,i,\ell}$, where $\ell \in \{0,1,\ldots,L-1\}$ is an integer lag index. \\
    ...
    \\
    \hline
    Source & \tref{TM-NCRF-Model}: NCRF Model \cite{DasBrodbeckSimonBabadi2020}\\
    \hline
    Ref.\ By & \ddref{DD-StimulusFeature}, \ddref{DD-NCRF-Coefficients}, \iref{IM-NeuralStimResponse}\\
    \hline
  \end{tabular}
  ...
  \begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
    \hline
    \rowcolor[gray]{0.9}
    Number& GD\refstepcounter{defnum}\thedefnum \label{GD-BackgroundActivity}\\
    \hline
    ...
    \\
    \hline
    Source & \cite{DasBrodbeckSimonBabadi2020} \\
    \hline
    Ref.\ By & \iref{IM-NeuralStimResponse}\\
    \hline
  \end{tabular}
\end{minipage}\\
...
\begin{minipage}{\textwidth}
  ...
  \begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
    \hline
    \rowcolor[gray]{0.9}
    Number& DD\refstepcounter{datadefnum}\thedatadefnum \label{DD-StimulusFeature}\\
    \hline
    Label& \bf Stimulus Feature Signal \\
    \hline
    Symbol &$e(t)$\\
    \hline
    SI Units & feature-dependent (commonly normalized or decibel-derived) \\
    \hline
    Equation&$e:[0,T] \rightarrow \mathbb{R}$, sampled at interval $\Delta t$\\
    \hline
    Description &
    The stimulus feature signal $e(t)$ is a scalar time series extracted from a continuous stimulus, such as a speech envelope. It is assumed to be time-aligned with the EEG/MEG recordings before NCRF estimation.\\
    \hline
    Sources& \cite{DasBrodbeckSimonBabadi2020, BrodbeckEelbrain} \\
    \hline
    Ref.\ By & \dref{GD-StimulusDriven}, \dref{GD-BackgroundActivity}, \iref{IM-NeuralStimResponse}\\
    \hline
  \end{tabular}
  ...
  \begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
    \hline
    \rowcolor[gray]{0.9}
    Number& DD\refstepcounter{datadefnum}\thedatadefnum \label{DD-NCRF-Coefficients}\\
    \hline
    ...
    \\
    \hline
    Sources& \cite{DasBrodbeckSimonBabadi2020} \\
    \hline
    Ref.\ By & \iref{ewat}\\
    \hline
  \end{tabular}
...
% \subsubsection{Data Types}\label{sec-datatypes}
...
\begin{minipage}{\textwidth}
  ...
  \begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
    \hline
    \rowcolor[gray]{0.9}
    Number& IM\refstepcounter{instnum}\theinstnum \label{ewat}\label{IM-NeuralStimResponse}\\
    \hline
    ...
    & For each source location $m$, orientation $i$, and time index $t$, the system first computes the stimulus-driven component by convolving the stimulus feature signal $e(t)$ with the NCRF coefficients $\tau_{m,i,\ell}$ over the lag range $\ell \in \{0,1,\ldots,L-1\}$. \\
    & It then adds the background activity term $v_{m,t,i}$ to obtain the source current estimate, that is, $j_{m,t,i}=\sum_{\ell=0}^{L-1}\tau_{m,i,\ell} e(t-\ell)+v_{m,t,i}$. \\
    & $j_{m,t,i}$ is the neuro source current at position $m$, orientation $i$, and time $t$. \\
    & $e(t)$ is the stimulus input feature signal, $\tau_{m,i,\ell}$ are the NCRF coefficients, and $v_{m,t,i}$ is the background source activity.\\
    \hline
    Sources& \cite{DasBrodbeckSimonBabadi2020} \\
    \hline
    Ref.\ By & None.\\
...
\begin{table}[!h]
  ...
  \begin{tabular}{
    ...
    & See Section~\ref{SecAuxConstants}
    & None \\
    ...
    & Small-magnitude coefficients (e.g., in $[-0.1,0.1]$)
    & Model-dependent \\

    $v_{m,t,i}$
    & Finite real values
    & Bounded variance
    & Zero-mean noise with variance approximately 1
    & High \\
...
%   \midrule
%   $T_W$ & $T_\text{init} \leq T_W \leq T_C$ (by~\aref{A-charge})
%   \\
%   \bottomrule
% \end{longtable*}
% \end{table}
...
\begin{itemize}
  ...
  \item[R\refstepcounter{reqnum}\thereqnum \label{R-Inputs}:]
    The system shall accept either (i) a precomputed continuous stimulus feature signal $e(t)$ or (ii) a raw continuous stimulus signal from which $e(t)$ can be extracted using a configured feature-extraction method (for example, speech-envelope extraction), and ensure the resulting $e(t)$ is time-aligned with EEG/MEG recordings.

  \item[R\refstepcounter{reqnum}\thereqnum \label{R-Calculate}:]
    The system shall compute source-level neural current estimates using the NCRF model by applying learned/estimated response functions to the stimulus feature signal $e(t)$, where $e(t)$ is the aligned feature signal accepted or derived in \rref{R-Inputs}. Verification of this computation shall be performed through the test cases and acceptance checks documented in the project V\&V plan (docs/VnVPlan/VnVPlan.tex).

  \item[R\refstepcounter{reqnum}\thereqnum \label{R-Calculate-Boosting}:]
    The system shall support TRF estimation via the boosting algorithm to preserve the baseline pipeline behavior.

  % \item[R\refstepcounter{reqnum}\thereqnum \label{R-VerifyOutput}:]
  ...
  \item[R\refstepcounter{reqnum}\thereqnum \label{R-Output}:]
    The system shall output (i) the estimated source currents $j_{m,t}$ and (ii) the NCRF coefficients $\tau_{m,i,\ell}$ in documented machine-readable formats suitable for downstream analysis in the existing pipeline, such as NumPy arrays, delimited tables, or BIDS-derivative files.
...
The system shall produce numerically stable results that can be verified against established TRF/NCRF benchmarks or reference implementations. Verification activities for this requirement are documented in the project V\&V plan (docs/VnVPlan/VnVPlan.tex).

\item[NFR2:] \textbf{Reproducibility}  
The system shall produce repeatable results for the same validated inputs, configuration parameters, and software version.

\item[NFR3:] \textbf{Maintainability}  
The system shall allow adding or modifying estimator algorithms without changing the input/output interfaces of the pipeline.

\item[NFR4:] \textbf{Portability}  
The system shall produce equivalent outputs for the same validated inputs and configuration when run on macOS, Windows, and Linux platforms, without requiring platform-specific changes to the documented user workflow.
...
\begin{itemize}
  ...
  \item[LC\refstepcounter{lcnum}\thelcnum\label{LC-OutputFormat}:]
  ...
  \item[LC\refstepcounter{lcnum}\thelcnum\label{LC-ModelParameters}:]
...
\begin{itemize}
  ...
  \item[LC\refstepcounter{lcnum}\thelcnum\label{LC-CoreMethods}:]
...
instance models with each other. Table~\ref{Table:R-trace} shows the
dependencies of instance models, requirements, and data constraints on each
other. Table~\ref{Table:A-trace} shows the dependencies of theoretical models,
  ...
  \begin{landscape}
    \begin{table}[h!]
      ...
      \begin{tabular}{|c|c|c|c|c|}
        ...
        \hline
        \tref{TM-NCRF-Model} & & X & X & X \\ \hline
        \dref{GD-StimulusDriven} & & X & X & X \\ \hline
        \dref{GD-BackgroundActivity} & & X & & \\ \hline
        \ddref{DD-NCRF-Coefficients} & & & X & X \\ \hline
        \iref{IM-NeuralStimResponse} & X & X & X & X \\ \hline
        \lcref{LC-OutputFormat} & & & & \\ \hline
        \lcref{LC-ModelParameters} & & & X & X \\ \hline
        \lcref{LC-CoreMethods} & & & & \\ \hline
      \end{tabular}
...
\begin{table}[h!]
  ...
  \begin{tabular}{|c|c|c|c|c|c|}
    \hline
    & \tref{TM-NCRF-Model} & \dref{GD-StimulusDriven} & \dref{GD-BackgroundActivity} & \ddref{DD-StimulusFeature} & \ddref{DD-NCRF-Coefficients} \\
    \hline
    \tref{TM-NCRF-Model} & & & & & \\ \hline
    \dref{GD-StimulusDriven} & X & & & & \\ \hline
    \dref{GD-BackgroundActivity} & X & & & & \\ \hline
    \ddref{DD-StimulusFeature} & & & & & \\ \hline
    \ddref{DD-NCRF-Coefficients} & X & & & & \\ \hline
    \iref{IM-NeuralStimResponse} & X & X & X & X & X \\ \hline
  \end{tabular}
...
\begin{table}[h!]
  ...
  \begin{tabular}{|c|c|c|c|c|}
    \hline
    & \rref{R-Inputs} & \rref{R-Calculate} & \rref{R-Output} & \iref{IM-NeuralStimResponse} \\
    \hline
    \rref{R-Inputs} & & & & X \\ \hline
    \rref{R-Calculate} & & & & X \\ \hline
    \rref{R-Output} & & & & X \\ \hline
    \iref{IM-NeuralStimResponse} & X & X & X & \\ \hline
  \end{tabular}
...
%     }
%     \caption{\label{Fig-ATrace} Traceability Matrix Showing the Connections Between Items of Different Sections}
%   \end{center}
% \end{figure}
...
%     }
%     \caption{\label{Fig-RTrace} Traceability Matrix Showing the Connections Between Requirements, Instance Models, and Data Constraints}
%   \end{center}
% \end{figure}
...
\section{Values of Auxiliary Constants}\label{SecAuxConstants}
...
\begin{itemize}
  ...
  \item $L_{\text{min}} = 1$ sample and $L_{\text{max}} = 500$ samples.
  \item Representative lag-window length: $L = 50$ samples.
  \item $e_{\text{max}} = 1$ for normalized stimulus features.
  \item $\tau_{\text{max}}$ is problem-dependent and selected during model configuration.
\end{itemize}
