...
\begin{itemize}
  ...
  \item[A1:] Preprocessed, time-aligned stimulus features and EEG/MEG recordings are available as inputs.
  \item[A2:] The stimulus feature representation is fixed and provided externally; feature extraction is outside the system scope.
  \item[A3:] Cortical source responses are modeled using linear temporal filters; nonlinear dynamics are not considered.
  \item[A4:] The cortical source space and the corresponding forward model are assumed to be fixed and known.
\end{itemize}
...
\item[NFR2:] \textbf{Usability}  
The system shall be usable by users with a background in neuroscience and Python programming, without requiring changes to the existing pipeline workflow.
  ...
  \begin{landscape}
    \begin{table}[h!]
      ...
      \begin{tabular}{|c|c|c|c|}
        ...
        \hline
        \tref{TM_NCRF_Model} & & X & X \\ \hline
        \dref{GD_StimulusDriven} & & X & X \\ \hline
        \dref{GD_BackgroundActivity} & & X & \\ \hline
        \ddref{DD_NCRF_Coefficients} & & & X \\ \hline
        \iref{IM_NeuralStimResponse} & & X & X \\ \hline
        \lcref{LC_OutputFormat} & & & \\ \hline
        \lcref{LC_ModelParameters} & & & X \\ \hline
